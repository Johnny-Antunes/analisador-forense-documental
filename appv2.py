...
def mapear_coluna(df, lista_sinonimos, excluir_se_conter=None):
    ...
    # 1. Busca exata por prioridade na lista de sinônimos
    ...
    # 2. Busca aproximada (substring) caso o cabeçalho venha com prefixos/sufixos
    for sin in lista_sinonimos:
        sin_clean = sanitizar_nome_coluna(sin)
...
def tratar_data_flexivel(val, nome_arquivo):
    ...
    # Corrige anomalia detectada: anos como '0026' convertidos para '2026'
...
def carregar_arquivos_para_sqlite(caminho_pasta_str):
    ...
    for idx_arq, arq in enumerate(arquivos):
        ...
        try:
            ...
            # Resolução Dinâmica de Colunas por Sinônimos
            ...
            # Localização: busca primeiro 'ocorrencia'; se não achar, busca 'origem'
            ...
            if (mode === "organico") {{
                ...
                blocos.forEach((grupo, grupoIdx) => {{
                    ...
                    grupo.forEach((node, idx) => {{
                        ...
                        updates.push({{ id: node.id, x: Math.round(cx + (internalRadius * Math.cos(ang))), y: Math.round(cy + (internalRadius * Math.sin(ang))), physics: false, borderWidth: (node.id === masterHubId ? 3 : 1), color: node.color }});
                    }});
                }});
                nodesDataSet.update(updates);
                setTimeout(() => {{ fitView(); if (currentHighlightId) applyHighlight(currentHighlightId); }}, 60);
