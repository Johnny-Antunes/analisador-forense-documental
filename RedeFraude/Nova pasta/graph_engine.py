    ...
    for _, row in df.iterrows():
        ...
        if cpf:
            ...
            G.add_node(
                nid, 
                tipo="cpf", 
                label=f"👤 {nome or formatar_cpf_cnpj(cpf)}", 
                valor=formatar_cpf_cnpj(cpf),
                nome_titular=nome or "NÃO INFORMADO"
            )
        ...
        if tel:
            ...
            G.add_node(
                nid, 
                tipo="telefone", 
                label=f"🚨 {formatar_tel(tel)}", 
                valor=formatar_tel(tel),
                nome_titular=""
            )
            entidades.append(nid)
        if placa:
...
# =====================================================
# 2. SUBGRAFO FORENSE & INTELIGÊNCIA RELACIONAL
# =====================================================
def processar_subgrafo_caso(G, cluster_nodes, filtro_tipos, hub_id, font_slider=12):
    ...
    for n in subG_filtrado.nodes:
        ...
        if nd["tipo"] == "cpf":
            tooltip_html = f"👤 TITULAR / SEGURADO\nNome: {nd.get('nome_titular', 'N/D')}\nCPF: {nd.get('valor', '')}\nVínculos no Caso: {grau} entidades"
        elif nd["tipo"] == "telefone":
            tooltip_html = f"🚨 CONTATO TELEFÔNICO\nNúmero: {nd.get('valor', '')}\nVínculos no Caso: {grau} entidades\n{'⭐ ÂNCORA CENTRAL DA CÉLULA' if is_hub else ''}"
        elif nd["tipo"] == "placa":
            tooltip_html = f"🚗 VEÍCULO\nPlaca: {nd.get('valor', '')}\nVínculos no Caso: {grau} entidades"
        else:
    ...
    for idx_e, (u, v, data) in enumerate(subG_filtrado.edges(data=True)):
        ...
        vis_edges.append({
            ...
            "width": largura,
            "smooth": False  # Linhas retas estilo i2
        })

    return vis_nodes, vis_edges

# =====================================================
# 3. RENDERIZADOR HTML VIS.JS (ALTA DISPERSÃO & RADIAL i2)
# =====================================================
...
<body>
    <div class="floating-controls">
        ...
        <button onclick="reorganizarEspacar()" title="Recalcular posições com alta dispersão">📐 Reorganizar</button>
    ...
    <script>
        ...
        // FÍSICA CALIBRADA: GRAVIDADE BAIXA E REPULSÃO ALTA ANTI-COLISÃO
        const options = {{
            physics: {{
                enabled: (initialLayout === "organico"),
                solver: 'barnesHut',
                barnesHut: {{
                    gravitationalConstant: -22000, // Repulsão dobrada: nós se afastam com força
                    centralGravity: 0.05,          // Gravidade central mínima: elimina o puxão para o meio
                    springLength: springDist,      // Comprimento da mola ajustável pelo usuário
                    springConstant: 0.02,          // Mola elástica e suave (não puxa com violência)
                    damping: 0.92,                 // Estabiliza rápido
                    avoidOverlap: 1.0              // 100% de repulsão de caixas delimitadoras
                }},
                stabilization: {{ 
                    enabled: true,
                    iterations: 220, 
        ...
        function fitView() {{
            network.fit({{ animation: {{ duration: 350 }}, padding: 50 }});
        }}
        ...
        function changeLayout(mode) {{
            ...
            if (mode === "organico") {{
                ...
                // LAYOUT RADIAL PEACOCK DO IBM i2 (ÓRBITAS CONCÊNTRICAS PERFEITAS)
                ...
                // Distribui o 1º Anel (Raio Médio)
                const r1 = Math.max(280, ring1.length * 28);
                ring1.forEach((n, i) => {{
                ...
                // Distribui o 2º Anel (Periferia Externa)
                const r2 = r1 + Math.max(260, ring2.length * 18);
                ring2.forEach((n, i) => {{
