...
Objetivo: Interface executiva da Mesa de Inteligência Forense de Fraudes em Assistências (LCFO).
          ...
          - Sidebar de Casos no Dashboard: ícone+texto alinhados à esquerda, dot de status
            semântico (paleta completa, não binário) alinhado à direita
          - Gaveta de Entidades: checkbox+texto à esquerda, pílula de tipo com a MESMA paleta
            de cores usada nos nós do grafo (CORES_POR_TIPO) alinhada à direita, ocupando
            toda a largura/altura disponível da gaveta redimensionável
...
# CORREÇÃO 4 (fallback seguro): CORES_POR_TIPO/PREFIXO_POR_TIPO agora têm um valor
# padrão definido aqui, para que a pílula de tipo na gaveta de Entidades (e
# construir_no_manual) nunca quebrem se enrich_engine.py estiver ausente -- e,
# mais importante, para que a pílula da lateral e o nó do grafo usem exatamente
# a mesma paleta sempre que enrich_engine.py estiver disponível.
try:
...
<style>
    ...
    /* CORREÇÃO 4: pílula de tipo com largura mínima -- não fica minúscula quando
       a gaveta é alargada, e cresce naturalmente com o texto do tipo. */
    .entity-tag-pill {
...
def instalar_atalhos_globais():
    """Atalhos globais de teclado (Ctrl+J busca, Ctrl+L notas, Ctrl+B alterna gaveta lateral)."""
    components.html("""
    <script>
    (function() {
...
def instalar_sidebar_resizable():
    """Habilita arraste contínuo na borda da sidebar (220px a 460px) com cursor ew-resize."""
    components.html("""
    <script>
    (function() {
...
def renderizar_painel_entidades(vis_nodes: List[Dict[str, Any]], identificador_caso: str, df_dados: pd.DataFrame):
    """
    Gaveta de Entidades/Add. CORREÇÃO 4: a pílula de tipo à direita agora usa
    CORES_POR_TIPO (mesma paleta dos nós no grafo, sem paleta duplicada), a
    lista ocupa altura maior (aproveitando a gaveta redimensionável até 460px
    de largura) e o texto do rótulo não é mais truncado artificialmente.
    """
    balde = obter_enrich_do_caso(identificador_caso)
    ...
    # CORREÇÃO 1: ícones restaurados nas abas Entities/Add
    ...
    with tab_ent:
        ...
        # CORREÇÃO 4: altura ampliada (era 420) -- a gaveta agora ocupa toda a
        # altura da tela (100vh via CSS), então a lista pode aproveitar mais
        # espaço vertical real em vez de ficar num recorte pequeno fixo.
        with st.container(height=640):
            for n in lista_exibicao:
                ...
                # Proporções ajustadas para dar mais espaço ao texto (o rótulo
                # não é mais truncado com [:16] -- flui na largura disponível,
                # que agora pode chegar a 460px com a gaveta esticada).
...
# ====================================================================
# 6. GAVETA LATERAL NO DASHBOARD (Cases)
# ====================================================================
def renderizar_drawer_lateral_dashboard():
    """
    CORREÇÃO 3: ordem trocada para bater com o Flowsint -- ícone+texto do caso
    alinhados à ESQUERDA, ocupando a maior parte da largura; ponto de status
    alinhado à DIREITA, isolado na ponta. A cor do ponto agora usa a paleta
    COMPLETA de CORES_STATUS (Em Investigação=azul, Monitoramento=roxo, etc.),
    não mais o binário verde/cinza de antes.
    """
    st.sidebar.markdown(
        """
    ...
    for c in cluster_info[:35]:
        ...
        with col_txt:
            if st.button(f":material/folder: {nome_c}", use_container_width=True, key=f"drw_case_{c['id']}"):
...
def renderizar_overview_caso(cluster_obj: Dict[str, Any], dados_caso_oficial: Dict[str, Any], identificador_caso: str):
    ...
    renderizar_drawer_lateral_dashboard()
...
if st.session_state["modo_descoberta_ativo"] and st.session_state["dados_descoberta"]:
    ...
    if aba_triagem == "Células e Redes da Blacklist":
        ...
        if not casos_filtrados:
            ...
            # CORREÇÃO 2: badge de risco movido para a MESMA linha do dot de
            # status (header), em vez de ficar implícito só nas stats abaixo.
            n_cols = 3
