...
# ====================================================================
    ...
    # -------------------------------------------------------------
    # SUB-ABA 4: CENTRO DE COMANDO PROATIVO & FUNIL DE ANOMALIAS
    # -------------------------------------------------------------
    else:
        st.markdown("#### Centro de Comando Proativo e Radar de Anomalias Territoriais")
        st.caption("Detecção de saltos volumétricos em municípios, atipicidades em serviços pet e abusos residenciais.")
        ...
        if df_anomalias.empty:
            ...
            # =====================================================
            # FUNIL DA TELA 4: INVESTIGAÇÃO CIRÚRGICA DE MUNICÍPIO
            # =====================================================
            st.markdown("---")
            st.markdown("##### Desdobramento Investigativo da Anomalia Municipal")
            st.caption("Isole uma praça em alerta para consolidar os principais operadores e abrir uma investigação ativa.")

            cidades_opcoes = {f"{r['cidade']}/{r['uf']} ({r['volume_atual']} acionamentos - {r['desvio_macro']})": r for _, r in df_anomalias.iterrows()}
            if cidades_opcoes:
                c_sel_label = st.selectbox("Selecione o Município em Anomalia:", list(cidades_opcoes.keys()))
                anomalia_alvo = cidades_opcoes[c_sel_label]

                from anomalias_engine import extrair_top_infratores_municipio
                ...
                if dados_infratores["total_ocorrencias"] > 0:
                    ...
                    with c_inf1:
                        st.markdown(f"**Top Titulares (CPFs):**")
                        st.dataframe(dados_infratores["top_cpfs"], use_container_width=True, hide_index=True)
                    with c_inf2:
                        st.markdown(f"**Top Telefones de Acionamento:**")
                        st.dataframe(dados_infratores["top_tels"], use_container_width=True, hide_index=True)
                    with c_inf3:
                        st.markdown(f"**Top Placas Reincidentes:**")
                        st.dataframe(dados_infratores["top_placas"], use_container_width=True, hide_index=True)

                    if st.button("Abrir Mesa de Investigação Ativa Desta Praça", type="primary", use_container_width=True):
                        st.session_state["modo_descoberta_ativo"] = True
                        st.session_state["dados_descoberta"] = {
                            "resumo": {
                                "alvo_buscado": f"Praça {anomalia_alvo['cidade']}/{anomalia_alvo['uf']}",
                    ...
                    st.info("Nenhuma ocorrência detalhada localizada para os critérios informados deste município.")
