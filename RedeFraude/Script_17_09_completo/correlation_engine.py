        ...
        for i in range(len(datas) - 2):
            ...
            if delta <= 60:
                ...
                if dias_desde_ultimo <= 60:
                    fatores.append(f"Reincidência residencial ATIVA: CPF {cpf_masc}, '{serv}' {len(datas)}x (último há {dias_desde_ultimo} dias)")
                elif dias_desde_ultimo <= 180:
                    fatores.append(f"Reincidência residencial moderada: CPF {cpf_masc}, '{serv}' {len(datas)}x (último há {dias_desde_ultimo} dias)")
                else:
                    fatores.append(f"Reincidência residencial histórica: CPF {cpf_masc}, '{serv}' {len(datas)}x (último há {dias_desde_ultimo} dias)")
                break
