...
with st.sidebar:
    ...
    with st.expander("🧹 3. Manutenção de Dados (Executar 1 vez)", expanded=False):
        st.caption("Corrige nomes de cidade/bairro/titular gravados com codificação corrompida (ex: SÃƒO PAULO -> SÃO PAULO).")
        if st.button("🔧 Reparar Codificação Histórica", use_container_width=True):
            with st.spinner("Varrendo e corrigindo textos já ingeridos no banco..."):
...
if st.session_state["modo_descoberta_ativo"] and st.session_state["dados_descoberta"]:
    ...
    score_disc, nivel_disc, cor_disc, fatores_disc, _ = calcular_score_caso(
        cluster_obj=None,
        df_assistencias=df_d,
        subgrafo=None
    )

    if nivel_disc == "BAIXO":
        titulo_desc, icone_desc = "Perfil Consultado — Sem Indicadores Relevantes", "🔍"
    elif nivel_disc == "MÉDIO":
        titulo_desc, icone_desc = "Perfil com Indicadores a Observar", "⚠️"
    else:
        titulo_desc, icone_desc = "Rede com Indicadores Críticos de Risco", "🚨"

    col_nav1, col_nav2 = st.columns([4.0, 1.0])
    with col_nav1:
        st.markdown(f"<div style='padding-top:4px;'><span style='color:#94A3B8; font-size:12px;'>Mesa de Triagem / </span> <b style='color:#FBBF24; font-size:14px;'>Investigação: {resumo_d['alvo_buscado']}</b></div>", unsafe_allow_html=True)
    with col_nav2:
    ...
    st.markdown(f"""
    <div class="hud-bar" style="border-color: {cor_disc};">
        <div style="display:flex; align-items:center; gap:20px; flex-wrap:wrap;">
            <span style="font-size:0.95rem; font-weight:700; color:{cor_disc};">
                {icone_desc} {titulo_desc}
            </span>
            <span style="font-size:0.82rem; color:#94A3B8;">
                Score: <b style="color:{cor_disc};">{score_disc}/100 ({nivel_disc})</b>
            </span>
            <span style="font-size:0.82rem; color:#94A3B8;">
                Acionamentos Diretos: <b style="color:#F8FAFC;">{resumo_d['assistencias_diretas']}</b>
            </span>
            <span style="font-size:0.82rem; color:#94A3B8;">
                Rede Total Descoberta: <b style="color:#38BDF8;">{resumo_d['total_assistencias']} assistências</b>
            </span>
            <span style="font-size:0.82rem; color:#94A3B8;">
    ...
    <div style="background: rgba(11, 17, 30, 0.95); border-left: 4px solid {cor_disc}; border-radius: 6px; padding: 8px 14px; margin-bottom: 12px; border-top: 1px solid #1E293B; border-right: 1px solid #1E293B; border-bottom: 1px solid #1E293B;">
        <div style="display: flex; align-items: center; justify-content: space-between;">
            <span style="color: {cor_disc}; font-weight: 700; font-size: 13px;">⚡ FRAUD SCORE: {score_disc}/100 [{nivel_disc}]</span>
            <span style="color: #94A3B8; font-size: 11px;">Indicadores Comportamentais Avaliados</span>
        </div>
    ...
    with st.expander("📌 Ações de Promoção e Vínculo", expanded=False):
        ...
        if tipo_acao == "🆕 Criar Novo Caso Oficial":
            ...
            with c_prom2:
                st_prom = st.selectbox("Status Operacional:", ["Confirmado Fraude", "Em Investigação", "Monitoramento Contínuo", "Sem Irregularidade Identificada"])
            with c_prom3:
    ...
    with t_d:
        cols_exib_disc = ["data", "id_assistencia", "servico", "titular", "cpf", "telefone", "placa", "bairro", "cidade", "uf"]
        if "empresa_cliente" in df_d.columns:
            cols_exib_disc.append("empresa_cliente")
        st.dataframe(df_d[[c for c in cols_exib_disc if c in df_d.columns]], use_container_width=True, hide_index=True)

# ====================================================================
# 7. TELA 1: PAINEL GERAL (CÉLULAS vs. BASE MESTRA vs. WATCHLIST CIDADES vs. TELA 4)
# ====================================================================
    ...
    aba_triagem = st.radio(
        "Navegação da Mesa:",
        ["🛡️ Células & Redes da Blacklist", "🎯 Base Mestra de Entidades Suspeitas", "📍 Watchlist de Municípios de Risco", "🌐 Centro de Comando & Radar Proativo (Tela 4)"],
        horizontal=True,
    ...
    if aba_triagem == "🛡️ Células & Redes da Blacklist":
        ...
        df_resumo_casos = df_resumo_casos.sort_values(by=["Score", "Tamanho"], ascending=[False, False])
        st.dataframe(df_resumo_casos, use_container_width=True, hide_index=True)
        st.caption("💡 Score preliminar (sem topologia completa do grafo) — abra o caso para o Fraud Score definitivo, que inclui a análise estrutural da rede.")

    # -------------------------------------------------------------
    # SUB-ABA 2: BASE MESTRA DE ENTIDADES (CADASTRO MANUAL & AUDITORIA)
    # -------------------------------------------------------------
    elif aba_triagem == "🎯 Base Mestra de Entidades Suspeitas":
        ...
        with f_c2:
            f_status = st.selectbox("Filtrar Status:", ["TODOS", "Ativo", "Confirmado Fraude", "Em Monitoramento", "Inativo", "Removido"])
        with f_c3:
        ...
        if suspeitos:
            ...
            with st.expander("🔎 Ver Histórico Detalhado de uma Entidade Específica"):
                ...
                with col_btn2:
                    motivo_rem = st.text_input("Motivo da remoção / arquivamento:", key=f"motivo_rem_{ent_alvo['id']}", placeholder="Ex: Terceiro inocente comprovado")
                with col_btn3:
                    st.write("")
    ...
    # -------------------------------------------------------------
    # SUB-ABA 3: WATCHLIST DE MUNICÍPIOS DE RISCO (CADASTRO & GESTÃO)
    # -------------------------------------------------------------
        ...
        with c_w_top1:
            ...
            st.caption("Cadastro de praças com predominância de fraude (ex: Lavras da Mangabeira, Santa Quitéria). Alimenta multiplicadores do Fraud Score.")
        ...
        with st.expander("➕ Cadastrar Novo Município na Watchlist", expanded=False):
            with st.form("form_nova_cidade_risco", clear_on_submit=True):
                ...
                with col_c1:
                    nova_cidade = st.text_input("Nome do Município:", placeholder="Ex: Lavras da Mangabeira")
                with col_c2:
                    novo_uf = st.selectbox("UF:", ["AC", "AL", "AP", "AM", "BA", "CE", "DF", "ES", "GO", "MA", "MT", "MS", "MG", "PA", "PB", "PR", "PE", "PI", "RJ", "RN", "RS", "RO", "RR", "SC", "SP", "SE", "TO"], index=5)
                
                col_m1, col_m2 = st.columns([3.0, 2.0])
                with col_m1:
                    novo_motivo_cid = st.text_input("Motivo / Padrão de Fraude Mapeado:", placeholder="Ex: Frequência atípica de guinchos e colusão regional")
                with col_m2:
                    novo_analista_cid = st.text_input("Analista Responsável:", placeholder="Seu nome", key="analista_cidade_input")
        ...
        if cidades_cadastradas:
            ...
            with st.expander("🗑️ Excluir Município da Watchlist"):
            ...
            st.info("Nenhum município cadastrado na Watchlist de Risco até o momento. Use o formulário acima para adicionar praças como Lavras da Mangabeira/CE ou Santa Quitéria/CE.")

    # -------------------------------------------------------------
    # SUB-ABA 4: TELA 4 - CENTRO DE COMANDO PROATIVO & ANOMALIAS MACRO
    # -------------------------------------------------------------
    else:
        st.markdown("#### 🌐 Centro de Comando Proativo & Radar de Anomalias Macro")
        st.caption("Detecção estatística de saltos volumétricos territoriais, conluios em serviços Pet e abusos residenciais.")

        with st.spinner("Calculando desvios volumétricos e baselines históricos de municípios..."):
        ...
        if df_anomalias.empty:
            ...
            with c_ano1:
                st.markdown(f"""<div class="kpi-card"><span style="color:#94A3B8; font-size:12px; font-weight:600;">MUNICÍPIOS COM ANOMALIA</span><br/><b style="font-size:24px; color:#EF4444;">{kpis_anomalias.get('total_anomalias', 0)}</b><br/><span style="font-size:10px; color:#94A3B8;">Mês ref: {mes_referencia}</span></div>""", unsafe_allow_html=True)
            with c_ano2:
                st.markdown(f"""<div class="kpi-card"><span style="color:#94A3B8; font-size:12px; font-weight:600;">ALERTAS PET ATIVOS</span><br/><b style="font-size:24px; color:#FBBF24;">{kpis_anomalias.get('alertas_pet', 0)}</b><br/><span style="font-size:10px; color:#94A3B8;">Saltos atípicos em clínicas</span></div>""", unsafe_allow_html=True)
            with c_ano3:
                st.markdown(f"""<div class="kpi-card"><span style="color:#94A3B8; font-size:12px; font-weight:600;">SALTOS RESIDENCIAIS</span><br/><b style="font-size:24px; color:#38BDF8;">{kpis_anomalias.get('alertas_res', 0)}</b><br/><span style="font-size:10px; color:#94A3B8;">Eletricista / Encanador</span></div>""", unsafe_allow_html=True)
            with c_ano4:
                st.markdown(f"""<div class="kpi-card"><span style="color:#94A3B8; font-size:12px; font-weight:600;">PRAÇAS NA WATCHLIST</span><br/><b style="font-size:24px; color:#A78BFA;">{kpis_anomalias.get('alertas_watchlist', 0)}</b><br/><span style="font-size:10px; color:#94A3B8;">Municípios monitorados</span></div>""", unsafe_allow_html=True)

            st.markdown("<br/>", unsafe_allow_html=True)

            tab_radar_mapa, tab_radar_tabela = st.tabs(["🗺️ Radar Territorial de Anomalias", "📋 Fila de Anomalias por Município"])
            ...
            with tab_radar_mapa:
                ...
                if not df_mapa_ano.empty:
                    ...
                    tooltip_ano = {
                        "html": "<div style='font-family:sans-serif; padding:5px;'><b style='color:#EF4444; font-size:13px;'>📍 {cidade}/{uf}</b><br/><b>Volume Atual:</b> {volume_atual} acionamentos<br/><b>Média Histórica:</b> {media_hist}<br/><b>Desvio:</b> {desvio_macro}<br/><b>Alertas:</b> <span style='color:#FBBF24;'>{alertas_str}</span></div>",
            ...
            with tab_radar_tabela:
                ...
                with col_f1:
                    filtro_tipo_ano = st.selectbox("Filtrar Categoria de Alerta:", ["TODOS", "🚨 Explosão Macro", "🐶 Anomalia Pet", "🏠 Salto Residencial", "📍 Watchlist de Risco", "⚠️ Pico sem Histórico"])
                with col_f2:
                ...
                for r_ano in df_view_ano.itertuples():
                    tabela_exibicao.append({
                        ...
                        "Meses Históricos": r_ano.meses_ativos_hist,
                        "Pet (Mês)": r_ano.pet_atual,
                        "Residencial (Mês)": r_ano.res_atual,
                        "Alertas Identificados": r_ano.alertas_str,
...
# =====================================================
    ...
    vis_nodes, vis_edges = processar_subgrafo_caso(
        ...
        id_caso=cluster_selecionado["id"]
    )

    max_bet_existente = max((n.get("betweenness", 0.0) for n in vis_nodes), default=0.0)
    subG_caso = G.subgraph(cluster_selecionado["nodes"])
    score_caso, nivel_caso, cor_caso, fatores_caso, _ = calcular_score_caso(
        cluster_obj=cluster_selecionado,
        df_assistencias=df_detalhes,
        subgrafo=subG_caso,
        betweenness_precalculado=max_bet_existente
    )
    ...
    <div class="hud-bar">
        <div style="display:flex; align-items:center; gap:18px; flex-wrap:wrap;">
            ...
            <span style="font-size:0.82rem; color:#94A3B8;">
                Score: <b style="color:{cor_caso};">{score_caso}/100 ({nivel_caso})</b>
            </span>
            <span style="font-size:0.82rem; color:#94A3B8;">
            ...
            <span style="font-size:0.82rem; color:#38BDF8;">
                Acionamentos: <b style="color:#38BDF8;">{total_assistencias} registros</b>
            </span>
            <span style="font-size:0.82rem; color:#94A3B8;">
    ...
    <div style="background: rgba(11, 17, 30, 0.95); border-left: 4px solid {cor_caso}; border-radius: 6px; padding: 8px 14px; margin-bottom: 12px; border-top: 1px solid #1E293B; border-right: 1px solid #1E293B; border-bottom: 1px solid #1E293B;">
        <div style="display: flex; align-items: center; justify-content: space-between;">
            <span style="color: {cor_caso}; font-weight: 700; font-size: 13px;">⚡ FRAUD SCORE: {score_caso}/100 [{nivel_caso}]</span>
            <span style="color: #94A3B8; font-size: 11px;">Indicadores Forenses Comportamentais & Topológicos</span>
        </div>
        <div style="color: #E2E8F0; font-size: 12px; margin-top: 4px;">
            {' • '.join(fatores_caso)}
        </div>
    ...
    def render_visao(nome_visao, altura_pixel=720, sufixo_key=""):
        if "Grafo" in nome_visao:
            components.html(html_codigo, height=altura_pixel)
            c_ginfo, c_gbtn, c_greset = st.columns([2.6, 1.3, 1.3])
            with c_ginfo:
                st.caption(f"Cluster Forense com {len(vis_nodes)} entidades ativas e {len(vis_edges)} conexões ponderadas.")
            with c_gbtn:
                st.download_button(
                    label="💾 Baixar Dossiê HTML",
                    data=html_codigo,
                    file_name=f"Dossie_Forense_{cluster_selecionado['id']}.html",
                    mime="text/html",
                    use_container_width=True,
                    key=f"dl_dossie_{cluster_selecionado['id']}_{sufixo_key}"
                )
            with c_greset:
                if st.button("🔄 Recalcular Layout", use_container_width=True, key=f"reset_layout_{cluster_selecionado['id']}_{sufixo_key}"):
                    resetar_layout_caso(cluster_selecionado["id"])
                    st.success("Layout resetado! Recalculando a organização do grafo...")
                    st.rerun()
            ...
            cols_tabela = ["data", "id_assistencia", "servico", "titular", "cpf", "telefone", "placa", "bairro", "cidade", "uf"]
            if "empresa_cliente" in df_detalhes.columns:
                cols_tabela.append("empresa_cliente")
            st.dataframe(df_detalhes[[c for c in cols_tabela if c in df_detalhes.columns]], use_container_width=True, hide_index=True)
            csv_dados = df_detalhes.to_csv(index=False).encode('utf-8')
            st.download_button(
                label="📥 Baixar Ocorrências em CSV",
                data=csv_dados,
                file_name=f"Assistencias_{cluster_selecionado['id']}.csv",
                mime="text/csv",
                key=f"dl_csv_{cluster_selecionado['id']}_{sufixo_key}"
            )
            ...
            if resumo_intel is None:
                ...
                if not df_novos_suspeitos.empty:
                    ...
                    with c_inc1:
                        if st.button("📥 Incorporar Novos Suspeitos e Anexar ao Caso", type="primary", use_container_width=True, key=f"btn_inc_{cluster_selecionado['id']}_{sufixo_key}"):
                            anexar_descoberta_a_caso_existente(
                                df_matches_criacao,
                                cluster_selecionado["id"],
                                analista=dados_caso.get("analista_responsavel", ""),
                                observacao="Incorporado via Expansão com Criações Diárias"
                            )
                            st.cache_data.clear()
                            st.success(f"✅ {len(df_novos_suspeitos)} novas entidades e suas assistências anexadas ao Caso {cluster_selecionado['id']} — a teia e a Tabela de Ocorrências foram atualizadas!")
                            st.rerun()
                    ...
                    with c_inc2:
                        ...
                        st.download_button(
                            ...
                            data=csv_suspeitos,
                            file_name=f"Novos_Suspeitos_{cluster_selecionado['id']}.csv",
                            mime="text/csv",
                            use_container_width=True,
                            key=f"btn_dl_susp_{cluster_selecionado['id']}_{sufixo_key}"
                        )
                else:
    ...
    if st.session_state["cfg_modo_exibicao"] == "📑 Abas Clássicas":
        ...
        with tab_g: render_visao("Grafo", altura_pixel=740, sufixo_key="aba")
        with tab_e: render_visao("Expansão", sufixo_key="aba")
        with tab_t: render_visao("Temporal", sufixo_key="aba")
        with tab_m: render_visao("Radar Territorial", sufixo_key="aba")
        with tab_d: render_visao("Tabela", sufixo_key="aba")
        with tab_caso:
            st.markdown(f"#### 📁 Identificação & Parecer da Investigação ({cluster_selecionado['id']})")
            c_f1, c_f2, c_f3 = st.columns([2.5, 1.5, 1.5])
            with c_f1: 
                novo_nome = st.text_input("Nome da Quadrilha:", value=dados_caso["nome_personalizado"], placeholder="Ex: Quadrilha Sapeaçu")
            with c_f2: 
                ...
                idx_status_atual = lista_status_caso.index(dados_caso["status"]) if dados_caso["status"] in lista_status_caso else 0
                novo_status = st.selectbox("Status:", lista_status_caso, index=idx_status_atual)
            with c_f3: 
                novo_analista = st.text_input("Analista:", value=dados_caso["analista_responsavel"])
            
            st.caption("💡 Use referências como @CPF_..., @TEL_... ou @PLACA_... no parecer — elas ganham destaque visual automático na Trilha de Auditoria.")
            ...
            st.caption("Remove a entidade da visualização deste caso específico, sem apagar nenhum dado bruto do banco.")

            col_oc1, col_oc2, col_oc3 = st.columns([2.5, 2.5, 1.5])
            with col_oc1:
                no_para_ocultar = st.selectbox("Entidade do caso:", cluster_selecionado["nodes"], key="sel_no_ocultar")
            with col_oc2:
                motivo_oc = st.text_input("Motivo da ocultação:", placeholder="Ex: Terceiro sem relação com o conluio", key="motivo_ocultar")
            with col_oc3:
                st.write("")
                if st.button("🙈 Ocultar Nó", use_container_width=True):
                    ocultar_no_do_caso(cluster_selecionado["id"], no_para_ocultar, motivo_oc, dados_caso.get("analista_responsavel", ""))
                    st.cache_data.clear()
                    st.success(f"Entidade {no_para_ocultar} ocultada deste caso.")
                    st.rerun()

            ocultos = listar_nos_ocultos_do_caso(cluster_selecionado["id"])
            if ocultos:
                st.markdown("**Nós ocultados nesta célula:**")
                for oc in ocultos:
                    c_r1, c_r2 = st.columns([4.0, 1.0])
                    with c_r1:
                        st.caption(f"• **{oc['node_id']}** — {oc['motivo'] or 'Sem justificativa'} (Ocultado em: {oc['data_ocultacao']})")
                    with c_r2:
                        if st.button("↩️ Restaurar", key=f"restaurar_{oc['node_id']}", use_container_width=True):
                            restaurar_no_do_caso(cluster_selecionado["id"], oc["node_id"])
                            st.cache_data.clear()
        ...
        with col_left:
            ...
            render_visao(st.session_state["cfg_painel_esquerdo"], altura_pixel=680, sufixo_key="esq")
        with col_right:
            ...
            render_visao(st.session_state["cfg_painel_direito"], altura_pixel=680, sufixo_key="dir")