...
# =====================================================
# COORDENADAS OFICIAIS DAS 27 CAPITAIS
# =====================================================
CAPITAIS_BRASIL = {
...
def formatar_mencoes_forenses(texto):
    """
    Converte referências como @CPF_..., @TEL_..., @PLACA_... digitadas nos pareceres
    em badges visuais destacados. Processamento puro de texto para HTML.
    """
    if not texto or not isinstance(texto, str):
        return texto or ""
    padrao = r'(@(CPF|TEL|PLACA)_[A-Za-z0-9]+)'
    return re.sub(
        padrao,
        r'<span style="background:#1E293B; color:#38BDF8; padding:2px 6px; '
        r'border-radius:4px; font-weight:600; font-size:12px; border:1px solid rgba(56,189,248,0.3);">\1</span>',
        str(texto)
    )
...
def corrigir_mojibake(texto):
    """
    Reverte corrupção de dupla codificação (UTF-8 lido como Windows-1252)
    e mapeia padrões literais observados em extrações corporativas.
    """
    if not texto or not isinstance(texto, str):
        return texto
    ...
    substituicoes = [
        ("SÃfO", "SÃO"), ("SÃƒO", "SÃO"), ("VIAMÃfO", "VIAMÃO"), ("VIAMÃƒO", "VIAMÃO"),
        ("GUARANTÃf", "GUARANTÃ"), ("GUARANTÃƒ", "GUARANTÃ"),
        ("Ã\"BIDOS", "ÓBIDOS"), ("LINDÃ\"IA", "LINDÓIA"),
        ("BRASÃLIA", "BRASÍLIA"), ("OLÃMPIA", "OLÍMPIA"),
        ("BÃRBARA", "BÁRBARA"), ("EPITÃCIO", "EPITÁCIO"),
        ("PAIÃ‡ANDU", "PAIÇANDU"), ("GONÃ‡ALO", "GONÇALO"),
        ("Ã‡", "Ç"), ("Ã§", "ç"), ("Ã‰", "É"), ("Ã©", "é"),
        ("ÃŠ", "Ê"), ("Ãª", "ê"), ("Ã“", "Ó"), ("Ã³", "ó"),
        ("Ã”", "Ô"), ("Ã´", "ô"), ("Ãš", "Ú"), ("Ãº", "ú")
    ]
    for err, cor in substituicoes:
...
def _buscar_coordenada_normalizada(cid_norm, uf_norm):
    """Busca com cache LRU para evitar escaneamentos repetitivos nos 5.570 municípios."""
    if not cid_norm:
        return None, None

    # 1. Match direto no IBGE
    if (cid_norm, uf_norm) in COORDENADAS_MUNICIPIOS:
        return COORDENADAS_MUNICIPIOS[(cid_norm, uf_norm)]

    # 2. Remove prefixos de sistema (ex: "A D BELO HORIZONTE", "CE SAO PAULO", "OSC JUIZ DE FORA")
    cid_sem_prefixo = re.sub(r'^[A-Z]{1,3}\s+(?:[A-Z]{1,2}\s+)?', '', cid_norm)
    if (cid_sem_prefixo, uf_norm) in COORDENADAS_MUNICIPIOS:
        return COORDENADAS_MUNICIPIOS[(cid_sem_prefixo, uf_norm)]

    # 3. Match por sufixo ou substring para distritos compostos
    if COORDENADAS_MUNICIPIOS:
        for (m_cid, m_uf), coords in COORDENADAS_MUNICIPIOS.items():
            if m_uf == uf_norm:
                if cid_norm.endswith(" " + m_cid) or cid_sem_prefixo.endswith(" " + m_cid):
                    return coords
                if len(m_cid) >= 6 and (m_cid in cid_norm or m_cid in cid_sem_prefixo):
                    return coords
...
def obter_coordenadas(cidade, uf):
    ...
    if not cid_norm:
        return None, None
    return _buscar_coordenada_normalizada(cid_norm, uf_norm)
