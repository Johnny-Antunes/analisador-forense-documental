...
from utils import (
    ...
    extrair_data_arquivo, formatar_cpf_cnpj, formatar_tel, normalizar_cidade,
    corrigir_mojibake, normalizar_uf_segura
)
...
def garantir_schema_db():
    ...
    try:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS caso_nos_ocultos (
                id_caso TEXT,
                node_id TEXT,
                motivo TEXT DEFAULT '',
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS entidades_suspeitas (
                ...
                analista TEXT,
                data_cadastro TEXT,
                data_remocao TEXT DEFAULT '',
                motivo_remocao TEXT DEFAULT '',
                UNIQUE(tipo, valor)
            )
        """)
        ...
        if colunas_suspeitas and "data_remocao" not in colunas_suspeitas:
            cursor.execute("ALTER TABLE entidades_suspeitas ADD COLUMN data_remocao TEXT DEFAULT ''")
        if colunas_suspeitas and "motivo_remocao" not in colunas_suspeitas:
            cursor.execute("ALTER TABLE entidades_suspeitas ADD COLUMN motivo_remocao TEXT DEFAULT ''")

        cursor.execute("PRAGMA table_info(assistencias)")
        colunas_assist = [r["name"] for r in cursor.fetchall()]
        if colunas_assist and "empresa_cliente" not in colunas_assist:
            cursor.execute("ALTER TABLE assistencias ADD COLUMN empresa_cliente TEXT DEFAULT ''")

        cursor.execute("PRAGMA table_info(criacoes_diarias)")
        colunas_criacao = [r["name"] for r in cursor.fetchall()]
        if colunas_criacao and "empresa_cliente" not in colunas_criacao:
            cursor.execute("ALTER TABLE criacoes_diarias ADD COLUMN empresa_cliente TEXT DEFAULT ''")
        ...
        # Tabela persistente dedicada para layout hierárquico / árvore
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS caso_layout_posicoes (
                id_caso TEXT NOT NULL,
                node_id TEXT NOT NULL,
                x REAL NOT NULL,
                y REAL NOT NULL,
                layout_tipo TEXT DEFAULT 'arvore',
                data_atualizacao TEXT,
                PRIMARY KEY (id_caso, node_id, layout_tipo)
            )
        """)

        cursor.execute("""
            CREATE TABLE IF NOT EXISTS cidades_risco (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def resetar_layout_caso(id_caso):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM caso_layout_nos WHERE id_caso = ?", (id_caso,))
        cursor.execute("DELETE FROM caso_layout_posicoes WHERE id_caso = ?", (id_caso,))
        conn.commit()
    finally:
        conn.close()


def salvar_posicoes_layout_caso(id_caso: str, posicoes: Dict[str, Dict[str, float]], layout_tipo: str = "arvore") -> bool:
    if not posicoes:
        return False
    conn = get_db_connection()
    try:
        ...
        dt_agora = datetime.now().strftime("%d/%m/%Y %H:%M")
        registros = [
            (id_caso, node_id, float(coords.get("x", 0)), float(coords.get("y", 0)), layout_tipo, dt_agora)
            for node_id, coords in posicoes.items()
        ]
        cursor.executemany("""
            INSERT INTO caso_layout_posicoes (id_caso, node_id, x, y, layout_tipo, data_atualizacao)
            VALUES (?, ?, ?, ?, ?, ?)
            ON CONFLICT(id_caso, node_id, layout_tipo) DO UPDATE SET
                x = excluded.x,
                y = excluded.y,
                data_atualizacao = excluded.data_atualizacao
        """, registros)
...
def carregar_posicoes_layout_caso(id_caso: str, layout_tipo: str = "arvore") -> Dict[str, Dict[str, float]]:
    conn = get_db_connection()
    posicoes = {}
    try:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT node_id, x, y FROM caso_layout_posicoes 
            WHERE id_caso = ? AND layout_tipo = ?
        """, (id_caso, layout_tipo))
        for r in cursor.fetchall():
            posicoes[r["node_id"]] = {"x": r["x"], "y": r["y"]}
    except Exception:
        pass
    finally:
        conn.close()
...
def ocultar_no_do_caso(id_caso, node_id, motivo="", analista=""):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        dt_agora = datetime.now().strftime("%d/%m/%Y %H:%M")
        cursor.execute("""
            INSERT OR REPLACE INTO caso_nos_ocultos (id_caso, node_id, motivo, analista, data_ocultacao)
            VALUES (?, ?, ?, ?, ?)
        """, (id_caso, node_id, motivo.strip(), analista.strip(), dt_agora))
        conn.commit()
    finally:
        conn.close()


def restaurar_no_do_caso(id_caso, node_id):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM caso_nos_ocultos WHERE id_caso = ? AND node_id = ?", (id_caso, node_id))
        conn.commit()
    finally:
        conn.close()


def listar_nos_ocultos_do_caso(id_caso):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT node_id, motivo, analista, data_ocultacao 
            FROM caso_nos_ocultos WHERE id_caso = ?
...
def obter_set_nos_ocultos(id_caso):
    if not id_caso:
        return set()
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT node_id FROM caso_nos_ocultos WHERE id_caso = ?", (id_caso,))
        return {r["node_id"] for r in cursor.fetchall()}
    except Exception:
        return set()
    finally:
        conn.close()
...
def cadastrar_cidade_risco(cidade, uf, motivo="", analista=""):
    cid_norm = normalizar_cidade(cidade)
    uf_norm = normalizar_uf_segura(uf)
    if not cid_norm or not uf_norm:
        return False, "Cidade e UF são obrigatórios."
    ...
    try:
        ...
        dt_agora = datetime.now().strftime("%d/%m/%Y %H:%M")
        cursor.execute("""
            INSERT INTO cidades_risco (cidade, uf, motivo, analista, data_cadastro)
            VALUES (?, ?, ?, ?, ?)
            ON CONFLICT(cidade, uf) DO UPDATE SET
                motivo = CASE WHEN excluded.motivo != '' THEN excluded.motivo ELSE cidades_risco.motivo END,
                analista = excluded.analista,
                data_cadastro = excluded.data_cadastro
        """, (cid_norm, uf_norm, motivo.strip(), analista.strip(), dt_agora))
        conn.commit()
        return True, f"✅ Município {cid_norm}/{uf_norm} registrado na Watchlist de Risco!"
...
def listar_cidades_risco():
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM cidades_risco ORDER BY uf ASC, cidade ASC")
        return [dict(r) for r in cursor.fetchall()]
    finally:
        conn.close()


def remover_cidade_risco(id_cidade):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM cidades_risco WHERE id = ?", (id_cidade,))
        conn.commit()
    finally:
        conn.close()


def obter_cidades_risco_set():
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT cidade, uf FROM cidades_risco")
        return {(r["cidade"].upper(), r["uf"].upper()) for r in cursor.fetchall()}
    except Exception:
        return set()
    finally:
        conn.close()
...
def obter_versao_cidades_risco():
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) as total, MAX(id) as max_id FROM cidades_risco")
        row = cursor.fetchone()
...
def reparar_mojibake_historico(progress_callback=None):
    conn = get_db_connection()
    total_corrigidos = {
        "assistencias_texto": 0, 
        "criacoes_diarias_texto": 0, 
        "assistencias_coords": 0, 
        "criacoes_diarias_coords": 0,
        "entidades_suspeitas": 0, 
        "cidades_risco": 0
    }
    try:
        cursor = conn.cursor()
        ...
        for tabela in ["assistencias", "criacoes_diarias"]:
            df_tab = pd.read_sql_query(f"SELECT id, cidade, uf, bairro, titular, servico FROM {tabela}", conn)
            if df_tab.empty:
                continue

            df_tab["cidade_fix"] = df_tab["cidade"].map(corrigir_mojibake)
            df_tab["uf_fix"] = df_tab["uf"].map(corrigir_mojibake)
            df_tab["bairro_fix"] = df_tab["bairro"].map(corrigir_mojibake)
            df_tab["titular_fix"] = df_tab["titular"].map(corrigir_mojibake)
            df_tab["servico_fix"] = df_tab["servico"].map(corrigir_mojibake)

            mudou_texto = (
                (df_tab["cidade_fix"] != df_tab["cidade"]) |
                (df_tab["uf_fix"] != df_tab["uf"]) |
                (df_tab["bairro_fix"] != df_tab["bairro"]) |
                (df_tab["titular_fix"] != df_tab["titular"]) |
                (df_tab["servico_fix"] != df_tab["servico"])
            )
            df_alterado = df_tab[mudou_texto]

            if not df_alterado.empty:
                lote_texto = list(df_alterado[["cidade_fix", "uf_fix", "bairro_fix", "titular_fix", "servico_fix", "id"]].itertuples(index=False, name=None))
                try:
                    cursor.executemany(f"""
                        UPDATE {tabela} SET cidade = ?, uf = ?, bairro = ?, titular = ?, servico = ?
                        WHERE id = ?
                    """, lote_texto)
                except sqlite3.IntegrityError:
                    for item in lote_texto:
                        try:
                            cursor.execute(f"UPDATE {tabela} SET cidade = ?, uf = ?, bairro = ?, titular = ?, servico = ? WHERE id = ?", item)
                        except sqlite3.IntegrityError:
                            pass
                total_corrigidos[f"{tabela}_texto"] = len(lote_texto)

            cursor.execute(f"SELECT DISTINCT cidade, uf FROM {tabela} WHERE cidade IS NOT NULL AND cidade != ''")
            cidades_distintas = cursor.fetchall()
            
            lote_geo = []
            for r_dist in cidades_distintas:
                cid, uf = r_dist["cidade"], r_dist["uf"]
                lat, lon = obter_coordenadas(cid, uf)
                if lat is not None:
                    lote_geo.append((lat, lon, cid, uf))
            ...
            if lote_geo:
                cursor.executemany(f"""
                    ...
                    WHERE cidade = ? AND uf = ?
                """, lote_geo)
                total_corrigidos[f"{tabela}_coords"] = len(lote_geo)

        cursor.execute("SELECT id, nome_referencia FROM entidades_suspeitas")
        lote_ent = [(corrigir_mojibake(r["nome_referencia"]), r["id"]) for r in cursor.fetchall()
                    if corrigir_mojibake(r["nome_referencia"]) != r["nome_referencia"]]
        if lote_ent:
            cursor.executemany("UPDATE entidades_suspeitas SET nome_referencia = ? WHERE id = ?", lote_ent)
            total_corrigidos["entidades_suspeitas"] = len(lote_ent)

        cursor.execute("SELECT id, cidade FROM cidades_risco")
        lote_cid = [(corrigir_mojibake(r["cidade"]), r["id"]) for r in cursor.fetchall()
                    if corrigir_mojibake(r["cidade"]) != r["cidade"]]
        if lote_cid:
            cursor.executemany("UPDATE cidades_risco SET cidade = ? WHERE id = ?", lote_cid)
            total_corrigidos["cidades_risco"] = len(lote_cid)
...
def investigar_alvo_em_criacoes_diarias(termo_busca):
    ...
    try:
        ...
        query_total = f"""
            SELECT DISTINCT data, id_assistencia, titular, cpf, telefone, placa, servico, bairro, cidade, uf, latitude, longitude, empresa_cliente
...
def promover_descoberta_para_caso(df_descoberta):
    ...
    try:
        ...
        for _, r in df_descoberta.iterrows():
            ...
            empresa = str(r.get("empresa_cliente", "")).strip() if pd.notna(r.get("empresa_cliente")) else ""
            ...
            lote_assistencias.append((
                ...
                str(r["bairro"]), str(r["cidade"]), str(r["uf"]), r.get("latitude"), r.get("longitude"), empresa
            ))

        cursor.executemany("""
            INSERT OR IGNORE INTO assistencias 
            (arquivo, id_assistencia, data, titular, cpf, telefone, placa, servico, bairro, cidade, uf, latitude, longitude, empresa_cliente)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, lote_assistencias)
        conn.commit()
...
def listar_entidades_suspeitas(filtro_tipo="TODOS", filtro_status="TODOS", termo_busca=""):
    ...
    try:
        ...
        if rows:
            cpfs = [r["valor"] for r in rows if r["tipo"] == "CPF"]
            tels = [r["valor"] for r in rows if r["tipo"] in ("TELEFONE", "TEL")]
            placas = [r["valor"] for r in rows if r["tipo"] == "PLACA"]

            contagens = {}
            def contar_em_lote(coluna, lista_valores):
                if not lista_valores:
                    return
                for chunk in [lista_valores[k:k+450] for k in range(0, len(lista_valores), 450)]:
                    ph = ','.join(['?'] * len(chunk))
                    q = f"""
                        SELECT val, SUM(cnt) as tot FROM (
                            SELECT {coluna} as val, COUNT(*) as cnt FROM assistencias WHERE {coluna} IN ({ph}) GROUP BY {coluna}
                            UNION ALL
                            SELECT {coluna} as val, COUNT(*) as cnt FROM criacoes_diarias WHERE {coluna} IN ({ph}) GROUP BY {coluna}
                        ) GROUP BY val
                    """
                    cursor.execute(q, chunk + chunk)
                    for r_c in cursor.fetchall():
                        contagens[r_c["val"]] = r_c["tot"]

            contar_em_lote("cpf", cpfs)
            contar_em_lote("telefone", tels)
            contar_em_lote("placa", placas)

            for r in rows:
                r["total_historico"] = contagens.get(r["valor"], 0)
        else:
            for r in rows:
                r["total_historico"] = 0
...
def remover_entidade_suspeita(id_entidade, motivo=""):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        dt_agora = datetime.now().strftime("%d/%m/%Y %H:%M")
        cursor.execute("""
            UPDATE entidades_suspeitas 
            SET status = 'Removido', data_remocao = ?, motivo_remocao = ?
            WHERE id = ?
        """, (dt_agora, motivo.strip(), id_entidade))
        conn.commit()
    finally:
        conn.close()
...
def consultar_todas_ocorrencias_entidade(tipo, valor):
    ...
    try:
        col = "cpf" if tipo == "CPF" else ("telefone" if tipo == "TELEFONE" else "placa")
        query = f"""
            SELECT 'Blacklist' as origem, data, id_assistencia, servico, titular, cpf, telefone, placa, bairro, cidade, uf, empresa_cliente
            FROM assistencias WHERE {col} = ?
            UNION ALL
            SELECT 'Criações Diárias' as origem, data, id_assistencia, servico, titular, cpf, telefone, placa, bairro, cidade, uf, empresa_cliente
...
def carregar_arquivos_para_sqlite(caminho_pasta_str, forcar_releitura=False):
    ...
    for idx_arq, arq in enumerate(arquivos_para_ler):
        ...
        try:
            ...
            c_empresa = mapear_coluna(
                df,
                ["nome_do_cliente", "nome_cliente", "cliente", "empresa_cliente", "seguradora", "seguradora_contratante", "companhia_cliente", "cia_cliente"],
                excluir_se_conter=["titular", "prestador", "garantia"]
            )
            ...
            s_empresa = df[c_empresa].map(normalizar_texto) if c_empresa else pd.Series([""] * n_rows)
            ...
            if not df_final.empty:
                ...
                cursor.executemany("""
                    INSERT OR IGNORE INTO assistencias (arquivo, id_assistencia, data, titular, cpf, telefone, placa, servico, bairro, cidade, uf, latitude, longitude, empresa_cliente)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, lote)
    ...
    for idx_arq, arq in enumerate(arquivos_para_ler):
        ...
        try:
            ...
            if not df_final.empty:
                ...
                antes = conn.total_changes
                cursor.executemany("""
                    INSERT OR IGNORE INTO criacoes_diarias (arquivo, id_assistencia, data, titular, cpf, telefone, placa, servico, bairro, cidade, uf, latitude, longitude, empresa_cliente)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, lote)
                inseridos_neste_arq = conn.total_changes - antes
                total_inseridos += inseridos_neste_arq
...
def consultar_detalhes_caso(cpfs, tels, placas):
    ...
    try:
        ...
        query = f"""
            SELECT data, id_assistencia, titular, cpf, telefone, placa, servico, bairro, cidade, uf, latitude, longitude, empresa_cliente
            FROM assistencias 
            WHERE {' OR '.join(clausulas)}
        """
...
def cruzar_com_criacoes_diarias(cpfs_conhecidos, tels_conhecidos, placas_conhecidas):
    ...
    for _, r in df_matches.iterrows():
        ...
        r_emp = str(r.get("empresa_cliente", "")).strip() if pd.notna(r.get("empresa_cliente")) else ""
