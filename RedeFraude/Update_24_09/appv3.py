...
Objetivo: Interface executiva da Mesa de Inteligência Forense de Fraudes em Assistências (LCFO).
          - HUD e Dossiê compactos centralizados horizontalmente no topo do grafo (Flowsint)
          - Arquitetura Full-Canvas exclusiva para o Grafo: 100% de largura e altura útil
          - Demais ecrãs (Dashboard, Overview, Watchlist, Base Mestra) com margens normais
          - Eliminação do duplo scroll na gaveta lateral (Entities): altura calculada em
            tempo real via JS (substitui o valor fixo calc(100vh - 170px))
          - Top Navbar 100% fixa e clicável (44px) com pílula de busca centralizada e atalho Ctrl+J
          - Modal central de busca tipo Command Palette (st.dialog)
          - Toggle panel da sidebar (Ctrl+B) direto no controle nativo (Zero Rerun)
          - Tooltips no rail esquerdo em CSS puro ([data-tooltip]) sem pílula vazia
          - Grafo abrindo sempre no modo padrão (Orgânico)
          - Scripts globais (atalhos de teclado, resize da sidebar) instalados uma
            única vez por sessão, não recriados a cada rerun
          - Todos os módulos analíticos, periciais e de dados mantidos na íntegra
"""
...
<style>
    ...
    .stDeployButton, .stAppDeployButton, #MainMenu, footer {
    ...
    /* TOP NAVBAR FIXA (44px) */
    div[class*="st-key-top_navbar_fixed"] {
    ...
    /* COLAPSO DE WRAPPERS HEIGHT=0 E COMPONENTES INVISÍVEIS */
    .block-container > div[data-testid="stVerticalBlock"] { gap: 0.5rem !important; }

    div[data-testid="stVerticalBlockBorderWrapper"]:has(div[class*="st-key-lcfo_left_rail"]),
    div.element-container:has(div[class*="st-key-lcfo_left_rail"]),
    div.element-container:has(> iframe[height="0"]),
    div.element-container:has(> div > iframe[height="0"]),
    div[data-testid="stVerticalBlockBorderWrapper"]:has(div[class*="st-key-top_navbar_fixed"]),
    div.element-container:has(div[class*="st-key-top_navbar_fixed"]) {
    ...
    /* Pílula de busca centralizada */
    div[class*="st-key-top_pill_search"] button {
    ...
    div[class*="st-key-top_pill_search"] button:hover {
    ...
    div[class*="st-key-top_navbar_fixed"] div[data-testid="stPopover"] > div > button,
    div[class*="st-key-top_navbar_fixed"] div[data-testid="stButton"] button {
        height: 32px !important;
    ...
    div[class*="st-key-top_navbar_fixed"] div[data-testid="stPopover"] > div > button:hover,
    div[class*="st-key-top_navbar_fixed"] div[data-testid="stButton"] button:hover {
        background-color: #242424 !important;
        border-color: #333333 !important;
        color: var(--primary-orange) !important;
    }

    /* Toggle Switch de Notas */
    ...
    /* =========================================================================
       HUD E DOSSIÊ COMPACTOS CENTRALIZADOS HORIZONTALMENTE
       ========================================================================= */
    div[class*="st-key-mesa_floating_hud"] {
        position: absolute !important;
        top: 50px !important;
        left: 50% !important;
        transform: translateX(-50%) !important;
        width: auto !important;
        min-width: min(92%, 1120px) !important;
    ...
    /* DOCK DE VISÕES COMPACTO CENTRALIZADO */
    div[class*="st-key-flowsint_view_dock"] {
        position: absolute !important;
        top: 96px !important;
        left: 50% !important;
        transform: translateX(-50%) !important;
    ...
    div[class*="st-key-flowsint_view_dock"] div[data-testid="stButton"] button {
    ...
    div[class*="st-key-flowsint_view_dock"] div[data-testid="stButton"] button:hover {
        background: #2A2A2A !important;
        color: var(--primary-orange) !important;
        border-color: #3A3A3A !important;
    }
    div[class*="st-key-flowsint_view_dock"] div[data-testid="stButton"] button[kind="primary"] {
        background: rgba(255, 115, 0, 0.20) !important;
        border-color: rgba(255, 115, 0, 0.5) !important;
        color: var(--primary-orange) !important;
    }

    /* RAIL FIXO DE ÍCONES (56px) */
    div[class*="st-key-lcfo_left_rail"] {
        ...
        z-index: 999998 !important; display: flex !important; flex-direction: column !important;
        align-items: center !important; padding: 12px 4px !important;
    }

    .rail-top-icons {
        display: flex; flex-direction: column; align-items: center; gap: 8px; width: 100%; margin-top: 6px;
    }
    .rail-bottom-icons {
        margin-top: auto; display: flex; flex-direction: column; align-items: center; width: 100%; padding-bottom: 6px; gap: 8px;
    }
    ...
    .rail-html-btn {
        ...
        font-size: 16px !important; cursor: pointer !important; transition: all 0.15s ease !important;
    }
    ...
    /* TOOLTIP CSS PURO (DISPARADO ESTRITAMENTE QUANDO EXISTE DATA-TOOLTIP NÃO-VAZIO) */
    div[class*="st-key-lcfo_left_rail"] button[data-tooltip]:not([data-tooltip=""]):hover::after {
    ...
    /* ELIMINAÇÃO TOTAL DA PÍLULA VAZIA */
    div[class*="st-key-lcfo_left_rail"] button:not([data-tooltip])::after,
    div[class*="st-key-lcfo_left_rail"] button[data-tooltip=""]::after {
        display: none !important;
    ...
    /* GAVETA LATERAL (SIDEBAR): SEM CAIXA INTERNA/BORDA BRANCA */
    [data-testid="stSidebar"] {
        left: 56px !important; background-color: var(--bg-surface) !important;
        border-right: 1px solid var(--border-subtle) !important;
        top: 0 !important; height: 100vh !important;
    }
    [data-testid="stSidebarContent"] { padding: 48px 4px 12px 4px !important; max-height: 100vh !important; }

    /* ELIMINAÇÃO DO DUPLO SCROLL: O SCROLL DO SIDEBAR FICA CONTROLADO */
    [data-testid="stSidebarContent"]:has(div[class*="st-key-entities_scroll_box"]) {
        overflow: hidden !important;
    }

    /* SCROLL ÚNICO PARA A LISTA DE ENTIDADES — altura real calculada via JS
       (instalar_calculo_altura_entidades), não mais um valor fixo em pixels,
       que não sabia descontar cards de Foco/Seleção quando aparecem. */
    [data-testid="stSidebar"] div[class*="st-key-entities_scroll_box"] {
    ...
    /* ELIMINA A BORDA BRANCA / RETÂNGULO INTERNO DA LISTA DE ENTIDADES */
    ...
    /* PADDING PADRÃO PARA TODAS AS TELAS (PRESERVA DASHBOARD, OVERVIEW, ETC.)
       position: relative é INCONDICIONAL para que o HUD/Dock flutuantes (absolute)
       sempre tenham a âncora correta, em qualquer view/tela (correção do bug de
       sobreposição ao trocar de view ou ativar o Cockpit dividido). */
    .block-container {
        position: relative !important;
        padding-top: 50px !important; padding-bottom: 1rem !important;
        padding-left: 1.2rem !important; padding-right: 1.2rem !important;
        margin-left: 56px !important; max-width: calc(100% - 56px) !important;
    }

    /* NEUTRALIZAÇÃO TOTAL DE WIDGETS */
    ...
    /* POPOVER TRANSLÚCIDO DO DOSSIÊ TOPOLÓGICO */
    ...
    div[data-testid="stPopoverBody"]:has(div.forensic-card) {
    ...
    /* HUD COMPACTO TRANSLÚCIDO */
    .hud-compact-row {
        background: rgba(27, 27, 27, 0.90) !important;
        border: 1px solid rgba(255, 255, 255, 0.12) !important;
        border-left: 3px solid var(--primary-orange) !important;
        border-radius: 8px; padding: 6px 14px; display: flex; align-items: center;
        justify-content: center; gap: 14px; flex-wrap: nowrap; white-space: nowrap; height: 38px;
    ...
    .forensic-card { background: rgba(24, 24, 24, 0.90); border: 1px solid rgba(255, 255, 255, 0.10); border-radius: 6px; padding: 12px 14px; height: 100%; }
...
def instalar_atalhos_globais():
    """
    Instalado uma única vez por sessão (o gate fica na Seção 6, via
    st.session_state). O listener de teclado e a injeção de tooltips ficam
    presos ao `window.parent`, então sobrevivem a qualquer rerun do Streamlit —
    reinjetar isto a cada rerun só recriava um iframe oculto sem necessidade,
    contribuindo para uma leve sensação de flicker.
    """
    components.html("""
    <script>
    (function() {
        if (window.parent.__lcfoKeydownHandler) {
            window.parent.document.removeEventListener('keydown', window.parent.__lcfoKeydownHandler, true);
        }
        window.parent.__lcfoKeydownHandler = function(e) {
            ...
            if (k === 'j') {
                e.preventDefault();
                e.stopPropagation();
                const btn = window.parent.document.querySelector('div[class*="st-key-top_pill_search"] button') ||
                            window.parent.document.querySelector('div[class*="st-key-btn_open_search_dialog"] button');
                if (btn) btn.click();
            } else if (k === 'l') {
                e.preventDefault();
                e.stopPropagation();
                const toggle = window.parent.document.querySelector('div[class*="st-key-top_toggle_notes_val"] label') ||
                               window.parent.document.querySelector('div[class*="st-key-top_toggle_notes_val"] input');
                if (toggle) toggle.click();
            } else if (k === 'b') {
                e.preventDefault();
                e.stopPropagation();
                const colBtn = window.parent.document.querySelector('[data-testid=stSidebarCollapseButton] button') ||
                               ...
                               window.parent.document.querySelector('button[aria-label*="sidebar" i]');
                if (colBtn) colBtn.click();
            }
        };
        window.parent.document.addEventListener('keydown', window.parent.__lcfoKeydownHandler, true);

        function aplicarTooltipsRail() {
            const map = {
                'rail_Casos': 'Células e Redes da Blacklist',
                'rail_Base Mestra': 'Base Mestra de Entidades Monitoradas',
                'rail_Watchlist': 'Watchlist de Municípios de Risco',
                'rail_Centro de Comando': 'Centro de Comando e Radar Territorial'
            };
            let todosAplicados = true;
            for (const [k, txt] of Object.entries(map)) {
                const btn = window.parent.document.querySelector(`div[class*="st-key-${k}"] button`);
                if (btn) btn.setAttribute('data-tooltip', txt);
                else todosAplicados = false;
            }
            return todosAplicados;
        }

        // Como isto agora roda uma única vez, usamos polling curto (em vez de um
        // único setTimeout) para garantir que funcione mesmo se o rail ainda não
        // tiver sido montado no instante exato da injeção.
        if (!aplicarTooltipsRail()) {
            let tentativas = 0;
            const poller = window.parent.setInterval(function() {
                tentativas += 1;
                if (aplicarTooltipsRail() || tentativas > 25) {
                    window.parent.clearInterval(poller);
                }
            }, 200);
        }
    })();
...
def instalar_sidebar_resizable():
    """
    Instalado uma única vez por sessão. A alça de redimensionamento fica anexada
    ao `[data-testid="stSidebar"]` real do DOM e sobrevive a qualquer rerun do
    Streamlit. Usa polling (em vez de uma checagem única) até a sidebar existir,
    para não falhar silenciosamente caso o primeiro disparo ocorra cedo demais.
    """
    components.html("""
    <script>
    (function() {
        if (window.parent.__lcfoSidebarResizableInit) return;
        window.parent.__lcfoSidebarResizableInit = true;

        function tentarInstalar() {
            const sidebar = window.parent.document.querySelector('[data-testid="stSidebar"]');
            if (!sidebar) return false;
            if (window.parent.document.getElementById('sidebar-drag-handle')) return true;
        ...
        if (!tentarInstalar()) {
            ...
            const poller = window.parent.setInterval(function() {
                tentativas += 1;
                if (tentarInstalar() || tentativas > 50) {
                    window.parent.clearInterval(poller);
                }
            }, 200);
        }
...
def instalar_calculo_altura_entidades(identificador_caso: str):
    """
    Substitui o valor fixo `calc(100vh - 170px)` por uma medição real do espaço
    livre abaixo da caixa de scroll de Entities — que varia conforme os cards de
    Foco/Seleção aparecem ou não. Ao contrário dos dois `instalar_*` acima, este
    PRECISA rodar a cada render (o conteúdo acima da caixa muda de tamanho), mas
    o custo de medir e ajustar um `style.height` é desprezível.
    """
    components.html(f"""
    <script>
    (function() {{
        function ajustarAlturaEntidades() {{
            const sidebarContent = window.parent.document.querySelector('[data-testid="stSidebarContent"]');
            const scrollBox = window.parent.document.querySelector('div[class*="st-key-entities_scroll_box_{identificador_caso}"]');
            if (!sidebarContent || !scrollBox) return;
            const contentRect = sidebarContent.getBoundingClientRect();
            const boxRect = scrollBox.getBoundingClientRect();
            const offsetTop = boxRect.top - contentRect.top;
            const margemInferior = 16;
            const alturaDisponivel = Math.max(160, sidebarContent.clientHeight - offsetTop - margemInferior);
            scrollBox.style.height = alturaDisponivel + 'px';
            scrollBox.style.maxHeight = alturaDisponivel + 'px';
        }}
        if (window.parent.__lcfoEntitiesResizeHandler) {{
            window.parent.removeEventListener('resize', window.parent.__lcfoEntitiesResizeHandler);
        }}
        window.parent.__lcfoEntitiesResizeHandler = ajustarAlturaEntidades;
        window.parent.addEventListener('resize', window.parent.__lcfoEntitiesResizeHandler);
        ajustarAlturaEntidades();
        setTimeout(ajustarAlturaEntidades, 120);
        setTimeout(ajustarAlturaEntidades, 400);
    }})();
    </script>
    """, height=0)
...
ICONE_TOOLBAR = {
    "Grafo de Vínculos": ":material/hub:",
    "Tabela de Ocorrências": ":material/table_chart:",
    "Expansão com Criações": ":material/swap_horiz:",
    "Radar Territorial (Mapa)": ":material/location_on:",
    "Evolução Temporal": ":material/timeline:",
    "Ferramentas": ":material/tune:"
}
...
if "_last_handled_reset_event" not in st.session_state: st.session_state["_last_handled_reset_event"] = ""
...
def renderizar_painel_entidades(vis_nodes: List[Dict[str, Any]], identificador_caso: str, df_dados: pd.DataFrame):
    ...
    with tab_ent:
        ...
        # Container com classe dedicada: elimina o duplo scroll e corre até ao fundo
        with st.container(key=f"entities_scroll_box_{identificador_caso}"):
            for n in lista_exibicao:
                tipo_chave = n.get("tipo", "cpf")
                cor_no = CORES_POR_TIPO.get(tipo_chave, {"bg": "#333333", "border": "#888888"})

                col_chk, col_txt, col_tag = st.columns([0.10, 0.62, 0.28])
                with col_chk:
    ...
    instalar_calculo_altura_entidades(identificador_caso)


# ====================================================================
# 3. RAIL FIXO ESQUERDO (56px) COM TOOLTIP CSS PURO & ZERO-RERUN TOGGLE
# ====================================================================
def renderizar_rail_fixo():
    with st.container(key="lcfo_left_rail"):
        ...
        for icone, aba_alvo, tooltip in NAV_PRINCIPAL:
            ...
            if st.button(icone, type=("primary" if ativo else "secondary"), key=f"rail_{aba_alvo}"):
                st.session_state["celula_ativa_id"] = None
                st.session_state["modo_descoberta_ativo"] = False
                st.session_state["dados_descoberta"] = None
        ...
        with st.popover(":material/database:"):
        ...
        # Botão de alternar sidebar nativo em HTML/JS (Zero Rerun)
        st.markdown("""
        <button class="rail-html-btn" data-tooltip="Alternar Painel Lateral (Ctrl+B)" onclick="
            const colBtn = window.parent.document.querySelector('[data-testid=stSidebarCollapseButton] button') ||
                           window.parent.document.querySelector('[data-testid=stSidebarCollapsedControl] button') ||
                           window.parent.document.querySelector('[data-testid=collapsedControl] button') ||
                           window.parent.document.querySelector('button[aria-label*=\\'sidebar\\' i]');
            if (colBtn) colBtn.click();
        ">
            <span style="font-family: 'Material Symbols Rounded', sans-serif; font-size:18px;">dock_to_left</span>
        </button>
        """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
# ====================================================================
# 4. MODAL CENTRAL DE BUSCA (COMMAND PALETTE TIPO FLOWSINT Ctrl+J)
# ====================================================================
dialog_decorator = getattr(st, "dialog", getattr(st, "experimental_dialog", None))

if dialog_decorator:
    @dialog_decorator("Search LCFO", width="small")
    def abrir_modal_busca_palette():
        st.markdown("<div style='font-size:11px; color:#888; margin-bottom:6px;'>Type a command or search...</div>", unsafe_allow_html=True)
        termo_cmd = st.text_input("Buscar:", placeholder="Search investigations, nodes, CPFs...", label_visibility="collapsed", key="search_dialog_input")
        
        if termo_cmd.strip():
            termo_norm = termo_cmd.strip().upper()
            vis_cache = st.session_state.get("_vis_nodes_atual_cache", [])
            achados_n = [n for n in vis_cache if termo_norm in (n.get("label", "") or "").upper() or termo_norm in (n.get("valor", "") or "").upper()][:6]
            if achados_n:
                st.markdown("<div style='font-size:10px; font-weight:700; color:#FF7300; margin-top:8px; margin-bottom:4px;'>ACTIVE GRAPH NODES</div>", unsafe_allow_html=True)
                for n in achados_n:
                    if st.button(f":material/hub: {n.get('label', n['id'])}", use_container_width=True, key=f"dlg_n_{n['id']}"):
                        st.session_state["entidade_foco_grafo"] = n["id"]
                        st.rerun()

            achados_c = [lbl for lbl in opcoes_celulas.keys() if termo_norm in lbl.upper()][:6]
            if achados_c:
                st.markdown("<div style='font-size:10px; font-weight:700; color:#FF7300; margin-top:8px; margin-bottom:4px;'>INVESTIGATIONS</div>", unsafe_allow_html=True)
                for lbl in achados_c:
                    if st.button(f":material/folder: {lbl}", use_container_width=True, key=f"dlg_c_{opcoes_celulas[lbl]}"):
                        abrir_caso_em_overview(opcoes_celulas[lbl])
        else:
            st.markdown("<div style='font-size:10px; font-weight:700; color:#888; margin-top:4px; margin-bottom:4px;'>INVESTIGATIONS</div>", unsafe_allow_html=True)
            for c in cluster_info[:5]:
                inf = casos_cadastrados.get(c["id"], {})
                nome_c = inf.get("nome") or c["hub_label"][:22]
                if st.button(f":material/fingerprint: {nome_c}", use_container_width=True, key=f"dlg_rec_{c['id']}"):
...
# ====================================================================
# 5. BARRA SUPERIOR (TOP NAVBAR) 100% FIXA - 44px
# ====================================================================
def renderizar_top_navbar(
    ...
    with st.container(key="top_navbar_fixed"):
        col_bc, col_busca, col_acoes = st.columns([1.8, 1.8, 1.4])

        with col_bc:
            if esta_em_caso:
                c_c1, c_sep, c_c2 = st.columns([0.48, 0.04, 0.48])
                with c_c1:
                    with st.popover(f"{nome_exibicao[:14] if nome_exibicao else identificador_caso} ⌵", use_container_width=True):
                        st.caption("Search investigations...")
                        termo_inv = st.text_input("Buscar caso:", placeholder="Nome...", label_visibility="collapsed", key=f"top_search_inv_{identificador_caso}")
                        casos_lista = [lbl for lbl in opcoes_celulas.keys() if not termo_inv or termo_inv.upper() in lbl.upper()][:8] if opcoes_celulas else []
                        for lbl in casos_lista:
                            cid = opcoes_celulas[lbl]
                            if st.button(f":material/folder: {lbl[:24]}", use_container_width=True, key=f"top_sw_case_{cid}"):
                                abrir_caso_em_overview(cid)
                with c_sep:
                    st.markdown("<span style='color:#555; font-size:14px; line-height:30px;'>/</span>", unsafe_allow_html=True)
                with c_c2:
                    visao_ativa = st.session_state.get("cfg_painel_unico", "Grafo de Vínculos")
                    with st.popover(f"{visao_ativa[:12]} ⌵", use_container_width=True):
                        if st.button(":material/arrow_back: Voltar ao Caso", use_container_width=True, key="pop_back_to_case"):
                            st.session_state["subtela_caso"] = "overview"
                            st.rerun()
                        st.markdown("<hr style='margin:4px 0; border-color:#333;'/>", unsafe_allow_html=True)
                        for opc in (OPCOES_TOOLBAR_OFICIAL if eh_oficial else OPCOES_TOOLBAR_DESCOBERTA):
                            ativo = (st.session_state["cfg_painel_unico"] == opc)
                            if st.button(f":material/hub: {opc}", use_container_width=True, type=("primary" if ativo else "secondary"), key=f"pop_vis_{opc}"):
                ...
                st.markdown(
                    ...
                    f"<span style='font-weight:800; color:#FF7300; font-size:14px;'>LCFO</span>"
                    f"<span style='color:#555;'>/</span>"
                    f"<span style='font-size:12px; color:#EDEDED; font-weight:600;'>{aba_nome}</span>"
                    f"</div>", unsafe_allow_html=True
                )

        with col_busca:
            with st.container(key="top_pill_search"):
                if st.button(":material/search: Search LCFO... (Ctrl+J)", use_container_width=True, key="btn_open_search_dialog"):
                    abrir_modal_busca_palette()

        with col_acoes:
            if esta_em_caso and eh_oficial:
                c_tog, c_split = st.columns([0.72, 0.28])
                with c_tog:
                    notas_atual = st.session_state["mostrar_notas_grafo"]
                    novo_notas = st.toggle("Toggle notes (Ctrl+L)", value=notas_atual, key="top_toggle_notes_val")
                    if novo_notas != notas_atual:
                        st.session_state["mostrar_notas_grafo"] = novo_notas
                        st.rerun()
                with c_split:
                    cockpit_on = st.session_state["cfg_cockpit_ativo"]
                    if st.button(":material/vertical_split:", use_container_width=True, type=("primary" if cockpit_on else "secondary"), help="Dividir Tela (Split View)", key="top_btn_split"):
...
# =====================================================
# instalar_atalhos_globais() e instalar_sidebar_resizable() escutam no
# window.parent, que sobrevive a qualquer rerun do Streamlit — então cada uma
# só precisa ser injetada uma vez por sessão, não a cada interação do usuário.
if not st.session_state.get("_atalhos_globais_instalados", False):
    instalar_atalhos_globais()
    st.session_state["_atalhos_globais_instalados"] = True

if not st.session_state.get("_sidebar_resizable_instalado", False):
    instalar_sidebar_resizable()
    st.session_state["_sidebar_resizable_instalado"] = True
...
# ====================================================================
# 7. GAVETA LATERAL NO DASHBOARD (Cases com Chaves Únicas)
# ====================================================================
...
# ====================================================================
# 8. MESA DE INVESTIGAÇÃO (CANVAS 2D & MÓDULOS)
# ====================================================================
def renderizar_mesa_investigacao(
    ...
    # PADRÃO ORGÂNICO RESTAURADO POR DEFEITO
    layout_atual_caso = st.session_state.get(f"layout_ativo_{identificador_caso}", "organico")

    if eh_oficial:
        vis_nodes, vis_edges = processar_subgrafo_caso(
            G=G, cluster_nodes=cluster_obj["nodes"] if cluster_obj else [],
            filtro_tipos=["telefone", "cpf", "placa"], hub_id=hub_id, font_slider=FONT_PADRAO,
            id_caso=identificador_caso, espacamento=ESPACAMENTO_PADRAO
        )
        
        # Só aplica coordenadas gravadas no SQLite se o utilizador ativou expressamente o modo Árvore
        if layout_atual_caso == "arvore":
            pos_banco = carregar_posicoes_layout_caso(identificador_caso, layout_tipo="arvore")
            if pos_banco:
                for n in vis_nodes:
    ...
    # =========================================================================
    # CONFIGURAÇÃO DE LARGURA TOTAL EXCLUSIVA PARA O GRAFO (SEM QUEBRAR OUTRAS TELAS)
    # =========================================================================
    eh_modo_grafo_puro = (
        st.session_state.get("cfg_painel_unico") == "Grafo de Vínculos" 
        and not st.session_state.get("cfg_cockpit_ativo", False)
    )

    if eh_modo_grafo_puro:
        st.markdown("""
        <style>
            .block-container {
                position: relative !important;
    ...
    # =========================================================================
    # HUD E DOSSIÊ COMPACTOS CENTRALIZADOS HORIZONTALMENTE (FLUTUANTES)
    # =========================================================================
    with st.container(key="mesa_floating_hud"):
        c_hud, c_dos = st.columns([0.80, 0.20], gap="small")
        with c_hud:
            st.markdown(f"""
            <div class="hud-compact-row">
                <span style="font-size:13px; font-weight:700; color:#EDEDED;">{nome_exibicao.upper()}</span>
                <span class="badge-pill" style="background:{cor}22; color:{cor}; border:1px solid {cor}55;">FRAUD SCORE: {score}/100 [{nivel}]</span>
                <span style="font-size:11px; color:#888;">Status: <b style="color:#6FA98A;">{status_atual}</b></span>
                <span style="font-size:11px; color:#888;">Âncora: <b style="color:#C9A66B;">{hub_id or 'N/D'}</b></span>
                <span style="font-size:11px; color:#888;">Acionamentos: <b style="color:#6C93B0;">{len(df_dados)} reg.</b></span>
                <span style="font-size:11px; color:#888;">Período: <b style="color:#CCCCCC;">{periodo_str}</b></span>
            </div>
            """, unsafe_allow_html=True)

        with c_dos:
            with st.popover(":material/analytics: Dossiê Topológico", use_container_width=True):
    ...
    # DOCK FLUTUANTE DE VISÕES (RENDERIZADO APENAS EM TELA ÚNICA)
    if not st.session_state["cfg_cockpit_ativo"]:
        ...
        with st.container(key="flowsint_view_dock"):
            cols_tb = st.columns(len(opcoes_toolbar), gap="small")
            for i, opc in enumerate(opcoes_toolbar):
                with cols_tb[i]:
                    ativo = (st.session_state["cfg_painel_unico"] == opc)
                    rotulo_icone = ICONE_TOOLBAR.get(opc, ":material/circle:")
                    if st.button(rotulo_icone, use_container_width=True, type=("primary" if ativo else "secondary"), help=opc, key=f"tbar_dock_{identificador_caso}_{i}"):
                        st.session_state["cfg_painel_unico"] = opc
                        st.rerun()
    ...
    html_codigo = gerar_html_grafo(
        ...
        layout_ativo=layout_atual_caso, espacamento=ESPACAMENTO_PADRAO
    )

    def render_modulo(nome_modulo, altura=880, sufixo_key=""):
        if "Grafo" in nome_modulo:
            ...
            if TEM_COMPONENTE_BIDIRECIONAL:
                evento_grafo = renderizar_grafo_bidirecional(
                    ...
                    layout_ativo=layout_atual_caso,
                    base_font_size=FONT_PADRAO, espacamento=ESPACAMENTO_PADRAO,
                    height=altura, key=f"comp_grafo_{identificador_caso}_{sufixo_key}"
                )
            else:
            ...
            if evento_grafo and isinstance(evento_grafo, dict):
                ...
                if tipo_ev == "node_click" and node_ev and node_ev != st.session_state.get("entidade_foco_grafo"):
                    ...
                    req_ts = evento_grafo.get("ts") or evento_grafo.get("timestamp") or 0
                    event_str = f"reset_{identificador_caso}_{req_ts}"
                    if event_str != st.session_state.get("_last_handled_reset_event"):
                        st.session_state["_last_handled_reset_event"] = event_str
                        st.session_state[f"layout_ativo_{identificador_caso}"] = "organico"
                        resetar_layout_caso(identificador_caso)
                        st.cache_data.clear()
                        st.rerun()

                elif tipo_ev == "save_layout_positions":
                    # PERSISTÊNCIA REAL NO SQLITE E NA SESSÃO — inclui tanto o payload
                    # completo enviado ao regenerar a Hierarquia quanto o payload
                    # parcial enviado por um simples arraste manual (dragEnd) em
                    # modo árvore; salvar_posicoes_layout_caso faz upsert por nó,
                    # então o recorte parcial não apaga o resto do layout salvo.
                    req_ts = evento_grafo.get("ts") or 0
                    event_str = f"pos_{identificador_caso}_{req_ts}"
                    if event_str != st.session_state.get("_last_handled_reset_event"):
                        st.session_state["_last_handled_reset_event"] = event_str
                        novo_layout = evento_grafo.get("layout", "organico")
                        st.session_state[f"layout_ativo_{identificador_caso}"] = novo_layout
                        posicoes_recebidas = evento_grafo.get("positions")
                        if posicoes_recebidas and novo_layout == "arvore":
                            salvar_posicoes_layout_caso(identificador_caso, posicoes_recebidas, layout_tipo="arvore")
            ...
            st.caption("Selecione uma linha para centralizar no Radar Territorial e destacar no Grafo.")
            try:
    ...
    if not st.session_state["cfg_cockpit_ativo"]:
        ...
        with col_conteudo:
            altura_modulo = 880 if eh_modo_grafo_puro else 740
            render_modulo(st.session_state["cfg_painel_unico"], altura=altura_modulo, sufixo_key="unico")
...
# ====================================================================
# 9. TELA DE OVERVIEW DO CASO (WIDGETS FLOWSINT)
# ====================================================================
def renderizar_overview_caso(cluster_obj: Dict[str, Any], dados_caso_oficial: Dict[str, Any], identificador_caso: str):
    ...
    with col_hdr2:
        c_fav, c_set = st.columns(2)
        with c_fav:
            st.button(":material/star_border:", help="Favoritar caso", key=f"fav_btn_{identificador_caso}")
        with c_set:
...
# ====================================================================
# 10. ROTEAMENTO DE TELAS PRINCIPAIS
# ====================================================================
