...
def resolver_identidade_componente(comp_nodes, conn=None):
    """
    Garante que células mantenham seu id_caso permanente.
    Reaproveita conexão ativa quando fornecida para evitar overhead de I/O.
    """
    fechar_ao_fim = False
    if conn is None:
        conn = get_db_connection()
...
def obter_versao_criacoes():
    """
    Gera uma impressão digital (fingerprint) ultrarrápida da base de criações.
    Se novos arquivos forem ingeridos, total ou max_id mudam imediatamente,
    invalidando o cache do radar automaticamente sem depender de limpeza manual.
    """
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
def obter_radar_expansoes_cached(ids_tupla, nos_serializados, versao_dados):
    """
    Cache inteligente: a presença de 'versao_dados' garante que o cache se
    autodestrua automaticamente sempre que criacoes_diarias receber dado novo.
    """
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
def obter_radar_expansoes(cluster_info):
    """
    Encapsulador que captura a impressão digital da base antes de consultar o cache.
    """
    if not cluster_info:
        return {}
