...
# =====================================================
# FUNÇÃO DE REPARO RETROATIVO DE MOJIBAKE
# =====================================================
def reparar_mojibake_historico(progress_callback=None):
    """
    Correção retroativa: repara textos já gravados no banco corrompidos por double-encoding.
    """
    conn = get_db_connection()
    total_corrigidos = {"assistencias": 0, "criacoes_diarias": 0, "entidades_suspeitas": 0, 
                        "casos_investigacao": 0, "cidades_risco": 0}
    try:
        cursor = conn.cursor()
        for tabela in ["assistencias", "criacoes_diarias"]:
            ...
            df_alterado = df_tab[mudou]
            if not df_alterado.empty:
                lote = list(df_alterado[["cidade_fix", "uf_fix", "bairro_fix", "titular_fix", "servico_fix", "id"]].itertuples(index=False, name=None))
                cursor.executemany(f"""
                    UPDATE {tabela} SET cidade = ?, uf = ?, bairro = ?, titular = ?, servico = ?
                    WHERE id = ?
                """, lote)
                total_corrigidos[tabela] = len(lote)
            if progress_callback:
                progress_callback(tabela, len(df_tab))
        ...
        cursor.execute("SELECT id_caso, nome_personalizado FROM casos_investigacao")
        lote_caso = [(corrigir_mojibake(r["nome_personalizado"]), r["id_caso"]) for r in cursor.fetchall()
                     if corrigir_mojibake(r["nome_personalizado"]) != r["nome_personalizado"]]
        if lote_caso:
            cursor.executemany("UPDATE casos_investigacao SET nome_personalizado = ? WHERE id_caso = ?", lote_caso)
            total_corrigidos["casos_investigacao"] = len(lote_caso)
...
def carregar_arquivos_para_sqlite(caminho_pasta_str, forcar_releitura=False):
    ...
    for idx_arq, arq in enumerate(arquivos_para_ler):
        ...
        try:
            ...
            c_cid = mapear_coluna(df, ["cidade_ocorrencia", "cidade_de_ocorrencia", "cidade_origem", "cidade_de_origem", "cidade", "cidade_de_origem"])
