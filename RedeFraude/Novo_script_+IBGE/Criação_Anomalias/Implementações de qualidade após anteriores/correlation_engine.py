...
def _analisar_transversalidade_seguradoras(df):
    """
    Transform 6: Detecta a mesma rede de entidades acionando assistências
    de mais de uma empresa cliente/seguradora distinta — sinal de fraude
    rotativa entre carteiras B2B, apostando que as seguradoras não cruzam dados.
    """
    pontos = 0.0
    fatores = []

    if "empresa_cliente" not in df.columns or df.empty:
        return 0.0, [], {"qtd_seguradoras_distintas": 0}

    seguradoras = sorted([str(s).strip() for s in df["empresa_cliente"].replace("", None).dropna().unique() if str(s).strip()])
    qtd = len(seguradoras)

    if qtd >= 3:
        pontos = 20.0
        fatores.append(f"🚨 Transversalidade crítica: rede aciona {qtd} seguradoras distintas ({', '.join(seguradoras[:4])})")
    elif qtd == 2:
        pontos = 12.0
        fatores.append(f"⚠️ Transversalidade B2B detectada: rede presente em 2 seguradoras ({', '.join(seguradoras)})")
