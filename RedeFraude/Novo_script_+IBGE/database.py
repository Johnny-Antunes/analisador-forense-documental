...
def garantir_schema_db():
    ...
    try:
        ...
        # Tabela de Pertencimento de Membros (Resolve a mutação de Hash)
        cursor.execute("""
        ...
        # Trilha Append-Only de Pareceres
        cursor.execute("""
        ...
        # Base Mestra de Entidades Suspeitas
        cursor.execute("""
        ...
        # Migração defensiva para garantir a coluna id_caso
...
# =====================================================
def resolver_identidade_componente(comp_nodes):
    """
    Garante que células mantenham seu id_caso permanente mesmo após crescerem
    ou incorporarem novas entidades ao longo do tempo.
    """
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
def promover_descoberta_para_caso(df_descoberta):
    """
    Insere assistências descobertas na tabela 'assistencias' e retorna
    o conjunto de nós prefixados para localização do ID real pelo grafo.
    """
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
def vincular_caso_e_entidades(id_caso_real, nos_promovidos, nome_quadrilha, status, analista, parecer):
    """
    Registra metadados e cadastra as entidades na Base Mestra vinculadas
    ao id_caso_real definitivo gerado pelo grafo.
    """
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
def listar_entidades_suspeitas(filtro_tipo="TODOS", filtro_status="TODOS", termo_busca=""):
    ...
    try:
        ...
        for r in rows:
            ...
            col = "cpf" if r["tipo"] == "CPF" else ("telefone" if r["tipo"] == "TELEFONE" else "placa")
            val = r["valor"]
            cursor.execute(f"""
                SELECT 
                    (SELECT COUNT(*) FROM assistencias WHERE {col} = ?) +
                    (SELECT COUNT(*) FROM criacoes_diarias WHERE {col} = ?) as total
            """, (val, val))
            r["total_historico"] = cursor.fetchone()["total"]
