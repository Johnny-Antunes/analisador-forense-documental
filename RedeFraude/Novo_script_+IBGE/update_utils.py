...
def limpar_cpf_cnpj(val):
    """Trata notação científica do Excel (E+10, E+13), floats simples e formatações com máscara."""
    if pd.isna(val): return ""
    v = str(val).strip()
    
    # 1. Trata notação científica
    if re.search(r"[eE][+-]?\d+", v):
        try:
            v = f"{int(float(v.replace(',', '.')))}"
        except Exception:
            pass
    # 2. Se for float simples sem formatação (Ex: "12345678901.0")
...
def limpar_tel(val):
    """Trata notação científica, floats e limpa telefones."""
    if pd.isna(val): return ""
    v = str(val).strip()
...
def limpar_placa(val):
    """
    Valida estritamente formatos reais brasileiros (Cinza e Mercosul).
    Descarta automaticamente placas mascaradas (ex: FGXXXXX, abXXXXX) ou dummies.
    """
    if pd.isna(val): return ""
    v = re.sub(r'[^a-zA-Z0-9]', '', str(val)).upper()
    if len(v) != 7:
        return ""
    # Padrão: 3 Letras + 1 Dígito + 1 Letra/Dígito + 2 Dígitos
