...
def obter_versao_cidades_risco():
    """
    Gera uma impressão digital (fingerprint) da tabela cidades_risco.
    Se municípios forem cadastrados ou excluídos, a versão muda,
    invalidando automaticamente o cache dos scores de triagem.
    """
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
def listar_entidades_suspeitas(filtro_tipo="TODOS", filtro_status="TODOS", termo_busca=""):
    """
    Lista as entidades suspeitas calculando o 'Histórico Total' em lote com apenas 3 queries.
    Preserva a coluna completa sem causar a lentidão de 600 queries individuais.
    """
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        ...
        # Contagem em lote (Batch query agrupada)
        if rows:
