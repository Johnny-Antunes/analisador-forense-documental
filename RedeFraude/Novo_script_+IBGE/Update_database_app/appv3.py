    ...
    with st.expander("🌐 2. Criações Diárias (Base Geral)", expanded=False):
        ...
        if st.button("🔄 Ingerir Criações no SQLite (Novos)", use_container_width=True):
            with st.spinner("Ingerindo arquivos novos de Criações..."):
                ...
                st.cache_data.clear()  # Defesa em profundidade: garante que o Radar de Alertas nunca fique defasado
                if qtd_reg_c > 0:
...
# ====================================================================
    ...
    if aba_triagem == "🛡️ Células & Redes da Blacklist":
        ...
        # =========================================================================
        # PONTO 1: Integração com correlation_engine.py na Fila de Triagem
        # =========================================================================
        ...
        # Ordenação por Risco Real (Score decrescente) e desempate por Tamanho
...
# =====================================================
    ...
    vis_nodes, vis_edges = processar_subgrafo_caso(
        ...
        id_caso=cluster_selecionado["id"]  # <-- Passa o id_caso para gravar e ler o layout do SQLite!
    )

    # =========================================================================
    # PONTO 2: Extração do betweenness já calculado no graph_engine (Zero custo extra)
    # =========================================================================
