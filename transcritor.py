...
# ESTA LINHA É A CHAVE: Ela configura o FFmpeg sem precisar de instalação no Windows
...
def gerar_relatorio_pdf(caminho_audio, resultado, hash_digital, nome_saida):
    ...
    pdf.cell(190, 10, "Relatorio de Transcricao de Audio - Mawdy", ln=True, align='C')
    pdf.ln(10)
    ...
    pdf.multi_cell(190, 7, f"Hash SHA-256: {hash_digital}")
    pdf.ln(5)
...
def executar_transcricao():
    if len(sys.argv) < 2:
        print("\n[!] Uso: python seu_script.py [ARRASTE O AUDIO AQUI]")
        return

    caminho_audio = sys.argv[1].strip('"')

    if not os.path.exists(caminho_audio):
        print(f"\n[X] Erro: Arquivo nao encontrado.")
        return

    print(f"\n[1/3] Garantindo integridade (Hash SHA-256)...")
    hash_digital = calcular_hash(caminho_audio)

    print(f"[2/3] Transcrevendo com IA... (Isso pode levar alguns minutos)")
    # O modelo 'base' é mais leve e rápido se o seu PC estiver sobrecarregado
    ...
    print(f"\n[V] SUCESSO! Relatorio gerado com integridade forense.")
    print(f"Caminho: {nome_pdf}")
