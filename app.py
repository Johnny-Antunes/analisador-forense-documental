...
def carregar_arquivos_para_sqlite():
    ...
    for idx_arq, arq in enumerate(arquivos):
        ...
        try:
            ...
            data_padrao = extrair_data_arquivo(arq.name)

            col_id = "NRO_ASSISTENCIA" if "NRO_ASSISTENCIA" in df.columns else None
            ...
            col_servico = "SERVICO" if "SERVICO" in df.columns else None
            ...
            for _, r in df.iterrows():
                ...
                dt = data_padrao
                if not dt and "ANO" in df.columns and "MES" in df.columns:
                    try: dt = f"{int(r['ANO'])}-{int(r['MES']):02d}-01"
                    except Exception: dt = ""
